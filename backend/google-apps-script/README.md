...
Este formulario reemplaza al Google Form que estaba incrustado antes. El formulario de `index.html` envía las solicitudes a un Web App de Google Apps
...
4. **Implementar → Gestionar implementaciones**:
   - Si ya había una implementación (por ejemplo, la URL que ya figura en
     ...
     versión* → **Implementar**. Así la URL no cambia y no hay que tocar `config.js`.
